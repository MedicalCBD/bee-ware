...
export class UpgradeUI {
  ...
  constructor(scene: Phaser.Scene, upgradeSystem: UpgradeSystem) {
    ...
    this.background = this.scene.add.rectangle(
      512, // Fixed center X (1024/2)
      384, // Fixed center Y (768/2)
      1024, // Fixed width
      768, // Fixed height
      0x000000, 0.7
    );
  ...
  private createUpgradeCards(upgrades: Upgrade[]): void {
    ...
    // Calculate starting X position to center all cards using fixed screen coordinates
    const startX = (1024 - totalWidth) / 2 + (cardWidth / 2);
    const startY = 384; // Fixed center Y
    ...
    upgrades.forEach((upgrade, index) => {
      ...
      cardBg.setScrollFactor(0); // Don't scroll with camera
      card.add(cardBg);
      ...
      descText.setScrollFactor(0); // Don't scroll with camera
      card.add(descText);
      ...
      buttonText.setScrollFactor(0); // Don't scroll with camera
      card.add(buttonText);
